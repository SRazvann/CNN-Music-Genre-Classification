...
def show_output(songname):
    ...
    st.write("## Predicted song genre: "+song_seg())
    ...
    ax.set_title(f"The {duration} extracted segments distribution probability in 5 seconds divisions.")
    plt.show()
    st.pyplot(fig)
    ...
    st.write('')
    st.write('')
    st.write("## Pie chart on song type probability")
    for i in class_labels:
    ...
    st.write('')
    st.write('')
    st.write("## Similar songs recommandation")
    ...
    st.write('')
    st.write('')
    st.write(f"## The 5 seconds divisions predictions. Total segments: {duration}")
    st.write(scores)

st.write(""" # Music Genre Recognition using a Convolutional Neural Network""")
...
file = st.sidebar.file_uploader("Upload an mp3 file or use the preloaded examples below:",
type=["mp3"])

model = GenreModel(input_shape=(288,432,3),classes=9)
model.load_weights(APP_FOLDER+'/Music_CNN.h5')

demo = st.sidebar.checkbox("Preloaded examples (1 for each genre)")

if(demo):
  
    song = st.sidebar.radio("Which Song you Want to check?",("BB King - The Thrill Is Gone","Johann Sebastian Bach - Toccata and Fugue in D Minor",
